...
/// A JSON-RPC request sent to the .NET bridge process.
...
/// A JSON-RPC response received from the .NET bridge process.
...
/// An error returned by the .NET bridge.
...
mod tests {
    ...
    fn test_request_serialization() {
        ...
        // params should be skipped when null
    ...
    fn test_request_serialization_with_params() {
        let req = Request {
            ...
            params: serde_json::json!({"file": "/tmp/test.al"}),
        };
        let json = serde_json::to_string(&req).unwrap();
        assert!(json.contains("\"params\""));
