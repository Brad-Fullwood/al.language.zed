...
//!
//! Provides static data about AL object types and runtime enum definitions,
...
/// Metadata for a single AL object type (e.g., Table, Page, Codeunit).
...
/// A runtime-only AL enum (not shipped in any .app package).
#[derive(Debug, Clone, Deserialize)]
...
/// Return all object type definitions.
pub fn object_types() -> &'static [ObjectType] {
    &OBJECT_TYPES
}

/// Return all runtime enum definitions.
pub fn runtime_enums() -> &'static [RuntimeEnum] {
    &RUNTIME_ENUMS
}

/// Look up an object type by its AL keyword (case-insensitive).
