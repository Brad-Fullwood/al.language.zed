...
async fn test_hover_latency() {
    ...
    // Warm up
    let _ = client.hover("objects/API/ItemJournalStaging.Table.al", 10, 10).await;

    let mut durations = Vec::with_capacity(ITERATIONS);
    for _ in 0..ITERATIONS {
        let start = Instant::now();
        let _ = client.hover("objects/API/ItemJournalStaging.Table.al", 15, 20).await;
    ...
    assert!(median_ms < 10.0, "hover must be <10ms, got {median_ms:.2}ms");
...
async fn test_completion_latency() {
    ...
    // Warm up
    let _ = client.completion("objects/Codeunit/IJLEventSubscribers.Codeunit.al", 10, 10).await;

    let mut durations = Vec::with_capacity(ITERATIONS);
    for _ in 0..ITERATIONS {
        let start = Instant::now();
        let _ = client.completion("objects/Codeunit/IJLEventSubscribers.Codeunit.al", 20, 15).await;
        durations.push(start.elapsed().as_micros() as u64);
    }

    let median_ms = median(&mut durations) as f64 / 1000.0;
    eprintln!("completion median: {median_ms:.2}ms ({durations:?})");
    assert!(median_ms < 20.0, "completion must be <20ms, got {median_ms:.2}ms");
...
async fn test_definition_latency() {
    ...
    let _ = client.definition("objects/API/ItemJournalStaging.Table.al", 10, 10).await;
    ...
    for _ in 0..ITERATIONS {
        let start = Instant::now();
        let _ = client.definition("objects/API/ItemJournalStaging.Table.al", 120, 30).await;
        durations.push(start.elapsed().as_micros() as u64);
    }
    ...
    assert!(median_ms < 20.0, "definition must be <20ms end-to-end, got {median_ms:.2}ms");
...
async fn test_document_symbols_latency() {
    ...
    // Warm up
    let _ = client.document_symbols("objects/API/ItemJournalStaging.Table.al").await;
    ...
    assert!(median_ms < 5.0, "document_symbols must be <5ms, got {median_ms:.2}ms");
...
async fn test_semantic_tokens_latency() {
    ...
    // Warm up
    let _ = client.semantic_tokens("objects/API/ItemJournalStaging.Table.al").await;
    ...
    assert!(median_ms < 15.0, "semantic_tokens must be <15ms, got {median_ms:.2}ms");
