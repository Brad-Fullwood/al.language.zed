    ...
    let code_with_lint = r#"codeunit 50100 "Diag Range"
{
    ...
    if let Some(file_diags) = diags.get(&uri) {
        for diag in file_diags {
            ...
            let start_line = start.get("line").and_then(|v| v.as_u64()).expect("start must have line");
            let start_char = start.get("character").and_then(|v| v.as_u64()).expect("start must have character");
            let end_line = end.get("line").and_then(|v| v.as_u64()).expect("end must have line");
            let end_char = end.get("character").and_then(|v| v.as_u64()).expect("end must have character");
            ...
            assert!(diag.get("severity").is_some(), "diagnostic must have severity: {diag}");

            // Diagnostic must have message
            let msg = diag.get("message").and_then(|v| v.as_str());
            assert!(msg.is_some() && !msg.unwrap().is_empty(),
    ...
    if let Some(file_diags) = diags.get(&uri) {
        for diag in file_diags {
            if let Some(code) = diag.get("code") {
                ...
                assert!(code_str.starts_with("AL-"),
    ...
    let code = r#"codeunit 50100 "Comp Kind"
{
    ...
    assert!(!items.is_empty(), "should have completions in procedure body");

    for item in &items {
        // Zed uses kind for icons — must be present
        assert!(item.get("label").and_then(|v| v.as_str()).is_some(),
    ...
    let code = r#"codeunit 50100 "Comp Detail"
{
    ...
    let helper = items.iter().find(|i|
        i.get("label").and_then(|l| l.as_str()) == Some("MyHelper"));
    if let Some(h) = helper {
        // Should have detail showing the signature
        let detail = h.get("detail").and_then(|d| d.as_str());
        assert!(detail.is_some(), "procedure completion should have detail: {h}");
        if let Some(d) = detail {
            assert!(d.contains("Integer") || d.contains("Text"),
    ...
    let code = r#"codeunit 50100 "Token Types"
{
    ...
    assert!(data.len() >= 10,
        "rich code should produce at least 10 token groups: got {}", data.len());
...
async fn test_completeness_d01_cross_file_hover_after_edit() {
    ...
    assert!(!symbols.is_empty(), "workspace should still find Helper CU after edit");
...
async fn test_completeness_d02_workspace_symbols_reflect_edits() {
    ...
    client.open_file("src/wsym_test.al", r#"codeunit 50100 "Original Name"
{
    procedure Foo()
    begin
    end;
    ...
    client.change_file("src/wsym_test.al", r#"codeunit 50100 "Changed Name"
{
    procedure Bar()
    begin
    end;
    ...
    let code = r#"codeunit 50100 "Close Diag"
{
    ...
    assert!(diags1.contains_key(&uri), "should have diagnostics after open");
...
async fn test_completeness_e02_signature_help_at_comma() {
    ...
    if let Some(result) = &sig {
        ...
        if let Some(idx) = active {
            assert!(idx >= 1, "at comma position, activeParameter should be >= 1: {result}");
        }
    }
...
async fn test_completeness_f01_folding_covers_all_object_types() {
    ...
    // Table
    client.open_file("src/fold_table.al", TABLE_WITH_FIELDS).await;
    ...
    assert!(fold_lines.len() >= 2, "table should have folds for fields and keys: {fold_lines:?}");
...
async fn test_completeness_f02_folding_ranges_are_line_based() {
    ...
    for fold in &folds {
        ...
        assert!(start_line.is_some(), "folding range must have startLine: {fold}");
        assert!(end_line.is_some(), "folding range must have endLine: {fold}");

        if let (Some(s), Some(e)) = (start_line, end_line) {
            assert!(e > s, "folding range must span at least 2 lines: start={s}, end={e}");
        }
    }
...
async fn test_completeness_g01_table_symbols_have_fields_as_children() {
    ...
    client.open_file("src/sym_table.al", TABLE_WITH_FIELDS).await;
    ...
    let table_sym = symbols.iter().find(|s|
        s.get("name").and_then(|n| n.as_str()).map(|n| n.contains("Test Item")).unwrap_or(false)
    );
    assert!(table_sym.is_some(), "should find 'Test Item' table symbol: {symbols:?}");

    if let Some(table) = table_sym {
        let children = table.get("children").and_then(|c| c.as_array());
        assert!(children.is_some(), "table symbol should have children (fields)");
        if let Some(kids) = children {
            ...
            assert!(total >= 5, "table outline should have at least 5 total descendant symbols: got {total}");

            // Every symbol at every level should have name, kind, range
            fn validate_symbols(syms: &[serde_json::Value]) {
                for child in syms {
                    assert!(child.get("name").is_some(), "symbol must have name: {child}");
                    assert!(child.get("kind").is_some(), "symbol must have kind: {child}");
                    assert!(child.get("range").is_some(), "symbol must have range: {child}");
...
async fn test_completeness_g02_codeunit_symbols_have_procedures_as_children() {
    ...
    let cu_sym = symbols.iter().find(|s|
        s.get("name").and_then(|n| n.as_str()).map(|n| n.contains("Helper")).unwrap_or(false)
    );
    ...
    if let Some(cu) = cu_sym {
        ...
        assert!(children.is_some(), "codeunit should have children (procedures)");
        if let Some(kids) = children {
            let names: Vec<&str> = kids.iter()
                .filter_map(|c| c.get("name").and_then(|n| n.as_str()))
                .collect();
            assert!(names.contains(&"Calculate"), "should have Calculate: {names:?}");
            assert!(names.contains(&"Validate"), "should have Validate: {names:?}");
        }
    }
...
async fn test_completeness_g03_enum_symbols_have_values() {
    ...
    let enum_sym = symbols.iter().find(|s|
        s.get("name").and_then(|n| n.as_str()).map(|n| n.contains("Item Status")).unwrap_or(false)
    );
    ...
    if let Some(e) = enum_sym {
        ...
        if let Some(kids) = children {
            let names: Vec<&str> = kids.iter()
                .filter_map(|c| c.get("name").and_then(|n| n.as_str()))
                .collect();
            assert!(names.iter().any(|n| n.contains("Active")), "should have Active: {names:?}");
            assert!(names.iter().any(|n| n.contains("Blocked")), "should have Blocked: {names:?}");
        }
    }
    ...
    let code = r#"codeunit 50100 "Rename Valid"
{
    ...
    let result = client.rename("src/rename_valid.al", 4, 10, "ItemCount").await;
    ...
    if let Some(changes) = edit.get("changes").and_then(|c| c.as_object()) {
        for (uri, edits) in changes {
            ...
            for e in edits {
                ...
                assert_eq!(new_text, Some("ItemCount"), "newText should be 'ItemCount': {e}");
            }
        }
    }
    ...
    let code = r#"codeunit 50100 "Rename Keyword"
{
    ...
    if let Some(edit) = &result {
        ...
        if let Some(c) = changes {
            assert!(c.is_empty(), "renaming a keyword should produce no changes: {edit}");
        }
    }
    ...
    let code = r#"codeunit 50100 "Refs Test"
{
    ...
    assert!(refs.len() >= 3, "MyVar should have at least 3 references: got {}", refs.len());
    ...
    let code = r#"codeunit 50100 "Cap Test"
{
    ...
    assert!(!acts.is_empty(), "codeAction capability must work on empty begin..end");
...
async fn test_completeness_k01_large_file_hover_is_correct() {
    ...
    assert!(all_names.len() >= 100,
        "large file should have 100+ symbols: got {}", all_names.len());
...
async fn test_completeness_l01_open_close_many_files() {
    ...
    // Open 10 files
    for i in 0..10 {
        let code = format!(r#"codeunit {id} "Multi {i}"
{{
    procedure Method{i}()
    begin
    end;
    ...
    assert!(syms.len() >= 10, "should find all 10 opened codeunits: got {}", syms.len());
    ...
    assert!(!syms.is_empty(), "should still find some codeunits after closing half");
