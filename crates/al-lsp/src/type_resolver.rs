...
//!
//! Not yet wired into production handlers (completions, hover) — currently
//! only exercised by unit tests. Will be integrated when type-aware
//! dot-completion is implemented.
...
impl<'a> TypeResolver<'a> {
    ...
    pub fn variables_at(&self, position: Position) -> Vec<VariableDecl> {
        ...
        if let Some(proc) = proc_node {
            ...
            // If this is a trigger, add implicit variables
    ...
    fn add_trigger_implicit_vars(&self, root: Node<'a>, result: &mut Vec<VariableDecl>) {
        ...
        // Rec — the current record (only for table-based objects)
    ...
    /// For page/report objects with SourceTable property, we'd need to parse
    /// properties — for now we return the object name for table objects.
