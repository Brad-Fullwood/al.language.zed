...
; --- Self Reference ---
; `this` keyword (e.g. this.ValidateCapacityUsageBuffer(...))
...
; --- Remaining Keywords (non-type keywords only) ---
; Type keywords are captured exclusively by @type.builtin below
...
; `namespace AdvaniaUK.Redco.MobileNav;` — keyword + dotted name
; `using Microsoft.Manufacturing.Document;` — keyword + dotted name
; The keyword (namespace/using) is a metadata_keyword, already captured by generic rule.
; Override: `using` → @keyword.import (falls back to @keyword in themes without it)
(namespace_or_using_declaration
  keyword: (metadata_keyword) @keyword.import
  (#match? @keyword.import "^using$"))
; Override: `namespace` → @keyword
(namespace_or_using_declaration
  keyword: (metadata_keyword) @keyword
  (#match? @keyword "^namespace$"))
; Highlight the dotted module path
(namespace_or_using_declaration name: (qualified_name) @type)
(namespace_or_using_declaration name: (name) @type)
