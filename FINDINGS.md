...
### Iteration 105 (2026-06-02)

Drained the new confirmed worklist: six related P1 unchecked `i64`→`i32` cast bugs across the daemon dispatchers (object/codeunit IDs silently wrapping on out-of-range input), plus the three companion test-gap findings (covered by the same regression tests). No standing open findings remained to drain (Open section was already empty).

| ID | Severity | Resolution |
|---|---|---|
| F-OPEN-230 | P1 | **Fixed (+ test-gap).** `dispatch_permissions` extracted `id` as i64 with no bounds check and passed it to `render_al`, so an out-of-range value would emit a `permissionset` declaration with an ID BC silently truncates/wraps. Now validates against i32 bounds and returns `INVALID_PARAMS` (preserving the 50100 default when absent). +2 tests. `crates/al-core/src/server/daemon/build_dispatch.rs`. |
| F-OPEN-231 | P1 | **Fixed.** `dispatch_tests_run` cast the `codeunit` param with `n as i32`, silently wrapping out-of-range IDs and running tests against the wrong codeunit. Now uses `i32::try_from` and returns `INVALID_PARAMS`. `crates/al-core/src/server/daemon/build_dispatch.rs`. |
| F-OPEN-232 | P1 | **Fixed (+ test-gap).** `dispatch_tests_run_batch` cast each `codeunitIds[]` entry with `n as i32` without range validation (error message only covered the type case). Now validates each entry with `i32::try_from`, reporting `codeunitIds[i] out of range`. +1 test. `crates/al-core/src/server/daemon/build_dispatch.rs`. |
| F-OPEN-233 | P1 | **Fixed (+ test-gap).** `dispatch_tests_last_results` cast `codeunitId` with `cu as i32` in both the single (codeunit, method) lookup and the bulk-filter path, silently wrapping out-of-range values (wrong / missing results). Both paths now validate via `i32::try_from` and return `INVALID_PARAMS`. +2 tests. `crates/al-core/src/server/daemon/build_dispatch.rs`. |
| F-OPEN-234 | P1 | **Fixed.** `dispatch_tests_snapshot_record` cast `codeunitId` with `n as i32` (impact currently limited — the dispatcher returns "not yet wired" — but the unchecked cast would become live once the bridge lands). Now validates via `i32::try_from`. `crates/al-core/src/server/daemon/build_dispatch.rs`. |
...
### Iteration 108 (2026-06-02)

Open backlog was empty. The worklist carried four new confirmed findings — three P1 bugs plus a P2 test-gap that is a companion to one of the P1s. All four were resolved this round (the test-gap is closed by the P1 fix's own regression test).

| ID | Severity | Resolution |
|---|---|---|
| F-OPEN-238 | P1 | **Fixed (+ tests).** `drain_events` (`crates/al-core/src/native_debug.rs`) recorded each Break in history with a location copied from the previous history entry (always empty in practice) rather than the current BC Break event. BC Break callbacks carry the location via their `StackFrame[]` argument (`[ApplicationObjectIdWrapper, StackFrame[], message]`), but `signalr_to_bc_event` (`crates/al-core/src/dap/bc_debug.rs`) discarded it. Extended `BcEvent::Break` with an `Option<BreakLocation>` extracted from the top frame's `SourcePosition` (PascalCase/camelCase tolerant, mirroring `native_dap::bc_stack_to_dap`) and used it when building `BreakpointHit`, so the user-visible debug `history` command reports the real line/column/procedure. +2 tests (top-frame extraction incl. object ids; camelCase fields); the existing no-args test now asserts `location == None`. |
| F-OPEN-239 | P1 | **Fixed (+ tests).** `get_cached_parse` (`crates/al-core/src/file_index.rs`) performed two non-atomic DashMap lookups; since `index_from_result` writes `file_trees` first and `files` last, a concurrent re-index could yield stale text paired with a fresh tree, so callers (e.g. references) converted tree byte offsets against the wrong source and produced wrong locations. A tree's root node spans its entire source, so `end_byte() == text.len()` is a coherence invariant: now re-reads until it holds (bounded spin/yield) and reports a miss rather than a torn pair under sustained churn. +2 tests (single-pass coherence; concurrent writers/readers stress that fails before the fix). |
...
### Iteration 112 (2026-06-02)

Two commits. Adversarial review of `crates/al-core/src/native_debug.rs` (the daemon-side wrapper around `BcDebugSession`, untouched since F-OPEN-238 closed the Break-location plumbing) plus a fresh `--all-targets -D warnings` clippy sweep.

| ID | Severity | Resolution |
|---|---|---|
| (chore) | — | Clippy `default_constructed_unit_structs` on `crates/al-core/src/syntax/lint.rs:101` — `LintConfig` is a unit struct, so `LintConfig::default()` tripped the new pedantic lint on `--all-targets`. Switched to a typed `Default::default()` binding that still exercises the `Default` impl once the struct grows real fields. |
| F-OPEN-244 | P2 | **Fixed (+ tests).** `NativeDebugSession.history` was an unbounded `VecDeque<BreakpointHit>` — `drain_events` pushed a hit per incoming BC Break event with no cap. Long debug sessions over hot loops (tests under a watchpoint, page repaint loops) accumulate thousands of hits per minute; the daemon's memory grew without bound. Added `HISTORY_CAP = 10_000` and a pure `push_with_cap` helper that FIFO-evicts the oldest entries past the cap (recent hits — the useful ones — are preserved). +4 unit tests: under-cap pass-through, at-cap FIFO eviction, the zero-cap edge case (documents saturating semantics), and a live-cap tripwire that catches any future "raise to u64::MAX"-style edit. `crates/al-core/src/native_debug.rs`. |
| F-OPEN-245 | P1 | **Fixed.** `step()` did not call `drain_events()` first, while `continue_exec()` does (F-014 pattern: "drain pending events so any Break that fired between the user's last command and continue is recorded in history before we tell BC to resume"). The same window applies to step — a Break event that landed on the SignalR push channel between commands was silently dropped from history when the user pressed step instead of continue. Added the symmetric drain at the top of `step()` with a doc comment tying it back to F-014. `crates/al-core/src/native_debug.rs`. |

Workspace test count: 2183 → 2187 (+4 history-cap regression tests). All gates green (`cargo fmt --all -- --check`, `cargo check --workspace --exclude zed-al`, `cargo clippy --workspace --exclude zed-al --all-targets -- -D warnings`, full test suite at 2187 passing).

### Iteration 113 (2026-06-02)

Two commits. Fresh adversarial review of `crates/al-core/src/semantic/host.rs` (CLR bridge orchestrator, untouched recently) and `crates/al-core/src/dap/bc_debug.rs` (the SignalR layer beneath `NativeDebugSession`).

| ID | Severity | Resolution |
|---|---|---|
| F-OPEN-246 | P2 | **Fixed.** `compile_bridge_from_source` (dev/sidecar bridge build path) assembled the DLL output path from a hardcoded `bin/Release/net8.0/` triple. Bumping the bridge's `<TargetFramework>` to `net9.0` (or later) silently broke the fallback with a misleading "Bridge built but output not found" error instead of a genuine compile failure. Now passes `--output bin/Release/rust-out` to `dotnet build` and reads from that same path so the location is TFM-independent. Production runs (which load the bridge from `OUT_DIR/bridge` baked in by `build.rs`) are unaffected. `crates/al-core/src/semantic/host.rs`. |
| F-OPEN-247 | P2 | **Fixed (+ regression tests).** `default_invoke_timeout` listed `ExpandVariableTree` / `ExpandLocalsTree` (no caller produces those strings). The real targets — `ExpandNode` (`expand_node`), `GetWatchNode` (`get_watch_node`), `UpdateBreakpoint` (`update_breakpoint`) — fell through to the 60s catch-all instead of the 30s bucket they belong in, so a hung variable-tree expand or breakpoint update took twice as long to surface as designed. Replaced the dead entries with the live ones; slotted `UpdateBreakpoint` into the breakpoint-operations bucket. +3 tests pinning the corrected mappings and turning the two dead-entry names into tripwires (must now fall to the 60s catch-all). `crates/al-core/src/dap/bc_debug.rs`. |
| F-OPEN-248 | P2 | **Fixed.** `invoke_with_timeout` computed `deadline = now() + timeout` AFTER acquiring `event_rx.lock().await`. When another invoke was holding the lock for its own (potentially 120s `Attach`) window, the queued caller's effective timeout silently extended by however long it waited for the mutex — the `default_invoke_timeout` budget was no longer honoured. Moved the deadline computation BEFORE the send + lock so the timer always starts when the caller's invoke begins. No new test added: the race is timing-dependent and exercising it deterministically requires a fake `event_rx` plus deliberate lock contention; the comment + code-inspection symmetry with the now-fixed timeout table covers the contract. `crates/al-core/src/dap/bc_debug.rs`. |
