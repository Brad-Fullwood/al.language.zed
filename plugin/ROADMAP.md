...
With `--wait-for-members` (`waitForMembers: true`), `object` and `by-id` run the
call-graph enrichment pass, so a workspace object carries its `fields` and
`methods` as a package object does. Without it they answer at once, and until
the call graph is built a workspace object comes back with `partial: true` and
no members. A file that will not parse degrades to object identity rather than
failing the lookup.
...
connection and keeps waiting while that index makes progress or the call graph
builds, up to 600 s.
