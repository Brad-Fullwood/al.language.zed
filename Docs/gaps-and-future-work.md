...
| B4 | 🟠 | Interpreter coverage | Missing: Date/Time/DateTime literals, compound operators (`+=`/`-=`), multi-variable declarations, scope-qualified enum members (`Enum::Member`), builtins `MaxStrLen`/`CreateDateTime`/`CurrentDateTime`. (List-of-T member access is now **done** — see B6.) | `crates/al-runtime/src/interpreter/tests_adversarial_wave2.rs` | Implement incrementally; each unblocks more `Interp` tests. |
