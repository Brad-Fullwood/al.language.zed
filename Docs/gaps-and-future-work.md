...
| C15 | ✅ | Analysis | DONE — graph-based affected-test detection (B7) **plus** indirect/polymorphic call resolution in the call graph (`al_insight::calls`), fed into both affected-test detection (`queries/tests.rs`, via `resolve_all_workspace_call_edges`) and coverage (`queries/test_coverage.rs`). Three indirect kinds now resolve, each as a new `EdgeKind::IndirectCall` (forward `caller → possible-target`): **(a) interface dispatch** — an `Interface "IFoo"`-typed variable's `.Bar()` → edges to `Bar` in *every* codeunit that `implements IFoo` (workspace `implements` is now captured in `register_workspace_nodes`); **(b) `Codeunit.Run(Codeunit::"X")` / `RunModal`** with a literal target → edge to `X.OnRun`; **(c) event publish site → subscribers** — a procedure invoking an event publisher → edges to each `[EventSubscriber]` handler. **Soundness:** these are deliberate **over-approximations** — sound for reachability (they only *add* edges, removing coverage/affected false-negatives at the cost of possible false-positives), kept distinct from `DirectCall` so consumers can tell resolved from over-approximated. **Still unresolved (known):** non-literal `Codeunit.Run(Var)` dispatch; direct/member call resolution remains name-based (overloaded names may over-credit). | `ROADMAP.md`; `al-insight/src/calls.rs`, `al-insight/src/index.rs`; `queries/test_coverage.rs`, `queries/tests.rs` |
