...
| C9 | 🟢 | Tests | DONE (dynamic coverage) — the interpreter now records **dynamic** statement & branch coverage as it executes, complementing the existing **static** call-graph coverage (`al-analysis`, unchanged). An opt-in, zero-cost-when-disabled `Coverage` collector lives behind `Option<Coverage>` on `DispatchCtx`: `eval_stmt` records each executed statement's line, and `eval_if`/`eval_case` record the branch decision. Level is honest — **statement coverage is complete** (a not-taken `if`/`case` body is never recorded); **branch coverage is two-way *decision* coverage** for `if`/`case` (then-taken vs else/no-arm), *not* per-`case`-arm path coverage or MC-DC, and loops are statement-level only. The `al-test` interpreter backend exposes this via `InterpMode::with_coverage(..)` + `coverage_report()` (per-file executed lines + branch tallies), aggregated across the run; the default `InterpMode::new(..)` stays static-only. Cross-file attribution works (callee statements attribute to the callee's file). `al-runtime` `interpreter::coverage` (+ `tests_coverage`), `al-test` `backends::interp`. **Remaining:** per-`case`-arm/condition-level coverage; expand mutators; make survival causes explicit when no interpreter-runnable test covers a mutant; surface dynamic coverage in the Cobertura emitter (today static-call-graph only). | `ROADMAP.md` (Native Test Runtime) |
