...
| C14 | ✅ | Docs/process | DONE — `Docs/architecture.md` adds Mermaid crate-dependency + request-flow diagrams (every edge derived from the real `crates/*/Cargo.toml` path-deps, not invented), and `Docs/testing-guide.md` documents per-layer verification (unit / native harness / GUI e2e / env-gated `alc`+semantic) plus a minimal reproducible-report template. `make repro-artifacts` regenerates `languages/al` and fails on any diff (the reproducible generated-artifact gate; also asserts `gen-zed-index` determinism). `make release-dryrun` is a read-only release-readiness gate — repo-slug + version/grammar-rev consistency (reusing `scripts/check-repo-consistency.sh` / `check-release-hygiene.sh`), build excl `zed-al` + the real `--features semantic` al-lsp, test, and `cargo publish --dry-run` for the 17 publishable crates — and never publishes. | `Docs/architecture.md`; `Docs/testing-guide.md`; `Makefile` (`repro-artifacts`, `release-dryrun`); `scripts/check-release-hygiene.sh` |
