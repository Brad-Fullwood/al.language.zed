...
| C6 | 🟠 | Symbols | **Partly done.** Deterministic Criterion benches now cover cold load (parse+index), warm lookup (by name/id + search), completion at a position, and the insight engine's graph build / `impact` / `trace`, against a fixed synthetic fixture — `crates/al-lsp/benches/perf.rs` (run via `cargo bench`, read instructions in `Docs/benchmarks.md`). **Honest time-vs-memory:** timings are Criterion-measured; the "+ memory data" is an **approximate** metric only (indexed symbol count + serialized-JSON bytes/symbol + insight node/edge counts via a one-shot `[C6 MEMORY]` line), **not** byte-level heap/RSS accounting. **Still open:** byte-level memory accounting; wiring the benches into a CI regression gate; distinguishing embedded-source vs generated-outline vs metadata-only in user output. | `ROADMAP.md` (Symbol Engine); `Docs/benchmarks.md` |
