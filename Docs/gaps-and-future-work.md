...
| B14 | 🟢 | Profiler accuracy | **DONE** — self-time now aggregates the profile's `timeDeltas` per node (`samples[i]` charged `timeDeltas[i]`, µs → ms; sums per sampled node id, guards mismatched array lengths). Falls back to the old 1 ms/hit estimate only when `samples`/`timeDeltas` are absent; `hit_count` is still reported and ranking is time-based. Remaining: `total_time_ms` still ≈ self-time (no call-tree roll-up) | `al-bc/src/profiling.rs` (`analyze_profile`); `queries/profiler_hints.rs` (`parse_profile`). NB: `al-explorer/src/views/profiler.rs` has a separate copy still on hit-count | Roll up `total_time_ms` via the call tree; unify the `al-explorer` copy onto the same aggregation. |
