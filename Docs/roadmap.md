...
- ✅ Verified native build gate: syntax/project/declaration/declared-binding/package-integrity
  diagnostics, shared across build surfaces, with atomic last-good artifact preservation.
...
- ✅ Real workspace dispatch plus executable InterpRecord backend, MockRecord, filters, and FlowFields.
- ✅ Graph-based affected-test selection and opt-in dynamic statement/decision coverage.
- 🟡 Replace pattern-based backend routing with AST/call-graph classification.
...
- ✅ §6 verified native compile (default): native syntax/project/declaration/declared-binding and
  package-integrity checks are wired; extend §1 into body-level expression/overload/control-flow
  parity.
...
- ✅ BC-server and NuGet downloads have bounded concurrency, same-package dedupe, retry/error handling,
  streaming size caps, validation, and atomic publication. `packageCachePath` / `appLocalFolderPaths`
  are wired across startup and LSP hot reload, with integration tests.
...
- ✅ Expose the complete shared daemon catalog through the zero-drift `al_call` MCP bridge; keep
  named tools as ergonomic aliases rather than an availability allow-list.
- 🟡 Add output-schema coverage and optional rich aliases where per-operation discovery materially
  improves agent use.
- 🟡 Include routing details in `al_runtests` output; add agent-oriented diagnostics for missing
  symbols/config/bridge/source.

### Zed UX → [README Zed integration](../README.md#zed-integration), [cli-and-tui](./features/cli-and-tui.md)
...
- ✅ Removed unsupported schema fields; `sessionId`/`breakOnNext` are consumed, and stateful agent
  control is shipped independently through MCP `al_debug`.
