...
| **Eliminate `with`** | `with_elimination.rs` | Expands `with Rec do begin X := Y end` into qualified `Rec.X := Rec.Y` (AA0205 compliance), resolving the record's table fields (including those contributed by its tableextensions) through the type resolver + symbol index. String literals and `//` comments are left untouched, and calls to the object's own procedures or AL built-ins are not qualified. |
