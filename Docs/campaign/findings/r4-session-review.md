...
- status: fixed without a lock: a re-indexed file now has each object and procedure name's owner list swapped under one entry lock, and only names the file no longer declares are pruned afterwards, so a request on another connection never finds the object missing (`re_indexing_a_file_never_hides_its_object_or_procedures` saw it missing on the old code within 2000 re-indexings). A request-wide read guard was rejected: one long request would hold off the refresh and, behind the waiting writer, every new request. Graphs a request already holds are `Arc`s and stay consistent; the refresh invalidates the cached ones for the next request.
