...
- status: fixed. 637d52ee removed the unreachable arm; the remaining contracts moved into `response_contract::validate` and the copy, `handles` and `validate_run_command_result` are gone. One validator governs every checked request, and a method without a contract fails with `no manual response contract is registered`.
