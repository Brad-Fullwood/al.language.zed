...
- status: fixed eebb9c98 — one `DeclLocCache` per request threads through both recorders, and the finished map is kept on `Workspace::code_lens_reference_counts` keyed to the generation revision and the open document. `record_member_bindings` also stopped reading only the file's first object declaration.
