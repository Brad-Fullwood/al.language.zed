...
2. `wasm32-wasip1` target installed: `rustup target add wasm32-wasip1`.
...
2. Build Zed extension WASM:
   1. `cargo build -p zed-al --target wasm32-wasip1 --release`
...
## Release Checklist
1. All tests pass (see `.claude/data/tasks.toml` WP11).
2. Grammar assets regenerated and committed.
3. Snippets and tasks updated and referenced in `extension.toml`.
4. `.claude/data/settings.md` and `.claude/data/feature-scope.md` updated.
