...
| B12 | Fix/Add | Verify every remaining DAP schema field is consumed, intentionally passed through, or documented as unsupported. | `debug_adapter_schemas/al.json` now says unsupported fields were removed, but it still carries many Microsoft-derived fields without an automated schema-to-config proof. | A schema-to-native-config test fails if a property is added without explicit consume/pass-through/unsupported handling; README/snippets/schema agree. |
...
| B17 | Add | Expand native-check and native diagnostics beyond the current `AL-NC001`-`AL-NC003` workspace checks. | `crates/al-analysis/src/queries/native_check.rs`, `crates/al-explorer/src/cli/commands/lsp/reports.rs`, and daemon method `nativeCheck` exist. | New native semantic rules have fixtures, documented diagnostic codes, CLI JSON output, and a clear decision on whether LSP/background diagnostics should surface them automatically. |
...
### P1/P2: Second-Pass Missed Items For Agent Assignment

These items were added after a deeper cross-surface pass over CLI docs, MCP docs, Zed tasks, native-check, snapshots, debug schemas, and process gaps.

| ID | Priority | Type | Task | Evidence / files | Acceptance check |
| --- | --- | --- | --- | --- | --- |
| D01 | P1 | Fix | Update MCP reference docs to list the tools that actually exist. | `Docs/reference/mcp-tools.md` still lists `al_suggestevent`, `al_testclassify`, `al_testcoverage`, and `al_depgraph` as planned even though `crates/al-lsp/src/server/mcp.rs` exposes them. | MCP docs are generated or tested against `tools/list`; no implemented tool is documented as planned. |
| D02 | P1 | Fix | Add `native-check` to the CLI command reference. | `crates/al-explorer/src/cli/args.rs` defines `NativeCheck`; `Docs/reference/cli-commands.md` does not mention it. | CLI reference includes syntax, JSON behavior, `AL-NC` diagnostic codes, and limitations for `native-check`. |
| D03 | P1 | Fix | Correct the `generate-completions` CLI reference wording. | `Docs/reference/cli-commands.md` describes completion or symbol data, while the command generates shell completion scripts. | Docs name the supported shells and stop implying AL completion data export. |
| D04 | P1 | Fix | Document `pack-native --validate`. | CLI exposes native package validation but `Docs/reference/cli-commands.md` omits the flag. | Reference docs show the flag, required toolchain, failure mode, and JSON metadata. |
| D05 | P1 | Fix | Correct `lint` wording so it does not imply a real native lint rule set. | `Docs/reference/cli-commands.md` says native plus Microsoft analyzers; the native lint registry is currently empty. | Docs say exactly which analyzer backend runs and whether native diagnostics are empty, partial, or disabled. |
| D06 | P1 | Fix | Clarify MCP lifecycle wording in README. | README says MCP bridge uses the shared daemon backend; the actual `al-lsp mcp` path runs in-process over stdio while sharing dispatch code. | README distinguishes daemon socket lifecycle from MCP stdio lifecycle. |
| D07 | P1 | Fix | Remove or qualify README claims about MCP-related debug schema fields. | README says snippets cover MCP-related debug fields; `debug_adapter_schemas/al.json` says unsupported MCP debug fields were removed. | README, snippets, and schema agree on every debug launch field. |
| D08 | P1 | Fix | Resolve the dotnet path documentation contradiction. | README says custom `dotnet` path config is unsupported; gaps/settings docs note `AL_DOTNET_PATH` exists. | Docs state the current supported path exactly: env var only today, plus planned `al.dotnetPath` if implemented. |
| D09 | P1 | Fix | Replace stale README build/package path references. | README still points to `crates/al-core/src/emit`; current emit code lives in `crates/al-emit`. | README architecture section names real crates and no deleted paths. |
| D10 | P1 | Fix | Correct the settings support table for `al.appLocalFolderPaths`. | `Docs/reference/settings.md` marks it as wired; gap docs and code review indicate it is parsed but not used for symbol loading. | Settings docs mark it partial until a fixture proves symbols load from configured local folders. |
| D11 | P1 | Fix | Clarify what background diagnostics actually include. | Settings docs say background diagnostics run after edits; compile/analysis behavior differs between native emit, official compiler validation, bridge diagnostics, and `native-check`. | Docs describe which diagnostics run automatically, which require explicit compile, and which require Microsoft tooling. |
| D12 | P1 | Fix | Refresh or explicitly version the debug adapter schema source. | `debug_adapter_schemas/al.json` says it was extracted from Microsoft AL Language extension v18. | Schema header names the current source version or marks the schema intentionally pinned to v18 with rationale. |
| D13 | P1 | Add | Add a schema-to-config consumption test for DAP launch fields. | Many debug schema properties are Microsoft-derived and native DAP support is partial. | Test fails when a schema property lacks consume, pass-through, or explicit unsupported metadata. |
| D14 | P1 | Fix | Stop encouraging passwords in argv for snapshot/profile commands. | `snapshot` and `profile` subcommands accept `--username` and `--password`, which can leak through shell history and process listings. | Commands support safer env, prompt, keyring, or password-file input; docs warn against argv secrets. |
| D15 | P1 | Add | Wire `test-snapshot record` to a live BC test run. | Snapshot subcommands currently include placeholder behavior instead of recording actual live results. | A fixture records a live or mocked BC run into a snapshot file with stable metadata. |
| D16 | P1 | Add | Make `test-snapshot replay` compare current behavior with a snapshot, not just load it. | Replay currently validates snapshot loading rather than rerunning and comparing behavior. | Replay returns pass/fail deltas for changed tests and has JSON output fixtures. |
| D17 | P1 | Add | Add a one-command snapshot diff from fresh run to baseline. | Current diff is file-to-file only, which requires manual orchestration. | CLI can run tests, compare to a baseline snapshot, and emit actionable deltas. |
| D18 | P1 | Fix | Implement or remove the `test-run-all --filter` behavior. | CLI help says the method-name filter is logged only in Phase 1. | The filter affects selected tests, or the flag/help is removed until it does. |
| D19 | P1 | Add | Route single-codeunit test runs through the interpreter when pure enough. | Batch routing has local interpreter ambitions; single-codeunit execution remains live-service oriented. | Pure codeunit tests can run locally with the same eligibility rules and clear fallback reasons. |
| D20 | P1 | Add | Add a Zed task for `native-check`. | CLI and daemon expose native-check, but `languages/al/tasks.json` does not. | Zed task runs native-check on the workspace and preserves JSON or readable diagnostics. |
| D21 | P1 | Add | Add an MCP tool for `native-check`. | MCP exposes diagnostics, tests, dependency graph, and reports, but no native-check tool. | `tools/list` includes `al_nativecheck` or equivalent with workspace input and JSON diagnostics. |
| D22 | P1 | Add | Document the `AL-NC` diagnostic namespace. | Native-check emits `AL-NC001` style codes, but user docs do not provide a reference. | Reference docs list each native-check code, severity, trigger, limitation, and sample fix. |
| D23 | P1 | Decide | Decide whether native-check should feed regular LSP diagnostics. | Daemon method `nativeCheck` exists separately from edit-time diagnostics. | A setting or design note states whether `AL-NC` results appear automatically, on save, on command, or never. |
| D24 | P1 | Add | Add a Zed task for `xlf refresh`. | CLI has more XLIFF commands than the generated extension task list. | Zed task invokes refresh with project-safe defaults and docs show its scope. |
| D25 | P1 | Add | Add a Zed task for untranslated XLIFF reporting. | CLI exposes untranslated reporting but Zed tasks only expose generation. | Task emits a focused untranslated report for the current workspace. |
| D26 | P1 | Add | Add a Zed task for XLIFF suggestion workflows. | Translation-memory suggestion workflows are not exposed from extension tasks. | Task runs suggestions with documented inputs and safe output handling. |
| D27 | P1 | Add | Add Zed tasks for affected-test reporting. | Affected-test functionality is CLI/daemon oriented but not surfaced as an extension task. | Users can run affected-test for current file or workspace from Zed with truthful limitations. |
| D28 | P1 | Add | Add Zed tasks for snapshot replay and diff. | Snapshot commands are CLI-only and currently underdeveloped. | Once snapshot behavior is real, Zed exposes replay/diff without manual command construction. |
| D29 | P1 | Add | Add a Zed task for running the test under the cursor or selected symbol. | Zed tasks include broad test commands but not a precise current-symbol test path. | Task maps `$ZED_SYMBOL`, file, and row to the intended test codeunit or method, with smoke coverage for argv. |
| D30 | P1 | Fix | Make workspace-mutating Zed fixup tasks default to preview or dry-run. | Tasks such as adding application areas, tooltips, and data classification can mutate a workspace directly. | Task labels and args make destructive behavior explicit; preview/dry-run is the first exposed action. |
| D31 | P1 | Add | Add a Zed task for native packaging with official validation. | `pack-native --validate` exists but is not easy to run from the extension task list. | Users can run package plus Microsoft validation from Zed with backend metadata in output. |
| D32 | P1 | Process | Generate a cross-surface feature matrix from actual code. | README and docs manually describe CLI, daemon, MCP, and Zed coverage and have drifted. | A generated matrix shows command availability per surface and fails CI on stale claims. |
| D33 | P1 | Process | Add a generator diff check for `languages/al/tasks.json`. | README says generated Zed files should come from templates, but task drift can happen manually. | CI regenerates tasks from the source template and fails on diff. |
| D34 | P2 | Fix | Clarify that MCP docs describe stdio tools, not a daemon socket API. | MCP docs say tools map through the daemon dispatcher, which can be read as requiring a running daemon. | Docs explain the shared dispatcher code path and the separate process boundaries. |
| D35 | P1 | Add | Include limitation metadata in MCP tool responses. | MCP currently wraps daemon JSON as text, and many reports are static, partial, or native-only. | Build, diagnostics, dead-code, impact, and coverage responses include backend and limitation fields. |
| D36 | P1 | Add | Add an MCP package/build artifact tool. | MCP has `al_build` but no explicit packaging workflow equivalent to native package. | Agents can request a package artifact path and validation status through MCP. |
| D37 | P1 | Add | Add source and auth options to `al_downloadsymbols` MCP input. | CLI symbol download has richer source/auth behavior than the MCP schema. | MCP supports server/NuGet source selection, credentials strategy, and concurrency/dedupe metadata. |
| D38 | P1 | Add | Add a workspace diagnostics MCP tool. | Existing MCP diagnostics are file-focused. Agents need project setup, symbol, toolchain, and config diagnostics. | MCP returns workspace-level setup diagnostics with remediation steps and machine-readable codes. |
| D39 | P1 | Add | Expand `al_runtests` MCP input to select codeunit, method, filter, and local-only mode. | MCP test running is narrower than CLI/test system capabilities. | Agents can request precise test scopes and receive fallback reasons when local routing is impossible. |
| D40 | P1 | Add | Add backend and validation controls to `al_build` MCP input. | Agents cannot clearly choose native emit versus Microsoft validation through MCP. | MCP input supports backend choice or validates config-derived backend, and output states what actually ran. |
| D41 | P1 | Fix | Wire `AppSourceCop.json` baseline fields into breaking/upgrade reports where possible. | Baseline report commands require explicit baseline symbol arrays; app projects often store baseline config in AppSourceCop. | Commands can discover baseline app/version from project config or explain exactly why they cannot. |
| D42 | P1 | Add | Publish examples for breaking and upgrade baseline inputs. | Empty-baseline behavior can look like success. | Docs include old `.app` baseline, JSON baseline, no-baseline, and missing-baseline examples with expected output. |
| D43 | P1 | Add | Add release asset checksum verification or a signed manifest. | Release comments note WASM download lacks SHA verification. | Release publishes checksums or signatures; installer validates where platform APIs allow it. |
| D44 | P1 | Process | Make release hygiene require a matching CI run for release/tag contexts. | `scripts/check-release-hygiene.sh` has optional `--require-ci`. | Release/tag workflow invokes the stricter mode by default and documents manual override policy. |
| D45 | P2 | Process | Add `cargo deny check` to the local readiness path or document it as CI-only. | `deny.toml` exists and CI covers supply-chain checks, but local review commands can miss it. | `make check` or release docs include dependency/license/advisory checks with install guidance. |
| D46 | P1 | Process | Add a scheduled or manual editor GUI E2E job. | `crates/al-test-harness/editor-e2e` exists, while normal tests do not prove Zed UI behavior. | CI can run editor E2E against a pinned fixture and publish artifacts/screenshots/logs. |
| D47 | P1 | Process | Add a Windows MCP stdio smoke test. | Windows builds run, but daemon transport and extension task parity are incomplete. | Windows CI starts `al-lsp mcp`, calls `tools/list`, and verifies basic tool schemas. |
| D48 | P1 | Fix | Hide or warn on daemon-backed Windows task paths until transport exists. | Daemon IPC is Unix-only while Windows artifacts are built. | Windows docs/tasks either work through a supported transport or clearly mark daemon-backed features unavailable. |
| D49 | P2 | Process | Consolidate or clearly separate `Docs/` and `docs/`. | The repo has both doc roots, and references point across them. | A docs index states ownership, or content moves to one root with redirects/links. |
| D50 | P2 | Fix | Mark old crate split and redesign docs as historical. | Split-plan docs can conflict with current architecture and active crate graph. | Historical docs have a banner and link to current architecture. |
| D51 | P1 | Add | Surface oversized source/object skips as diagnostics. | Source indexing and package source extraction can skip oversized or unavailable content. | Users and agents get a warning with file/object identity and the effect on navigation/static reports. |
| D52 | P1 | Add | Surface package load failures in a workspace doctor report. | Symbol and package loading can fail partially while later reports still run with incomplete data. | Doctor/diagnostics output lists failed packages, missing dependencies, and stale caches. |
| D53 | P1 | Add | Add config validation for unknown, inert, or partial settings. | Settings can be parsed, ignored, or only partially supported across surfaces. | A CLI/MCP doctor command flags unknown settings and known partial settings such as local folder symbols. |
| D54 | P2 | Process | Add performance regression gates for hot static-analysis queries. | Benchmarks exist but are not clearly used as a regression gate. | Scheduled CI compares benchmark or fixture timings with thresholds for symbol indexing, impact, dead code, and formatting. |
| D55 | P1 | Fix | Implement profiler total-time roll-up. | Gap docs say profile `total_time_ms` remains self-time and lacks call-tree roll-up. | Profiling output distinguishes self time and inclusive total time with tests. |
| D56 | P1 | Fix | Replace literal-substring architecture lint matching with structured or regex matching. | Gap docs say the architecture lint is based on literal substrings and not regex/AST-level rules. | Lint rules support reliable matching with fixtures for false positives in comments and literals. |
| D57 | P1 | Add/Fix | Expand mutation testing with more mutators, timeout policy, and equivalent-mutant handling. | Mutation support exists but parallelism and semantic breadth are limited. | Mutant reports include skipped/equivalent/timeouts and cover at least arithmetic, boolean, branch, and AL-specific mutators. |
| D58 | P1 | Add | Add confidence metadata to dead-code and impact reports when source bodies are unavailable. | Symbol packages often lack method bodies, affecting static call graphs. | Reports include confidence or source-availability fields so agents do not over-trust incomplete graphs. |
| D59 | P1 | Add | Add a symbol cache freshness and invalidation command. | Symbol cache behavior affects diagnostics, navigation, dependencies, and baselines. | CLI/MCP can show cache roots, package versions, stale entries, and provide a safe refresh/invalidate path. |
| D60 | P1 | Add | Feed app.json `idRanges` validation into regular diagnostics or quick checks. | Native-check can report object IDs outside ranges, but users must know to run it. | Either LSP diagnostics surface range violations or docs/tasks make the explicit check unavoidable. |
| D61 | P2 | Simplify | Normalize `project`, `workspace`, and file argument names across CLI commands. | Commands use varied names and defaults, which complicates task generation and agent invocation. | Public help follows a documented naming convention and task templates map consistently. |
| D62 | P1 | Add | Document symbol source precedence and cache behavior with tests. | Settings include package cache, `.alpackages`, local folders, NuGet feeds, and BC server download paths. | Reference docs and fixtures prove precedence, fallback order, and conflict behavior. |
| D63 | P1 | Add | Add a semantic bridge/toolchain doctor section. | Bridge, `dotnet`, compiler, symbols, and official AL tooling failures currently surface in different ways. | One command reports installed versions, configured paths, missing tools, and whether native-only mode is active. |
| D64 | P2 | Process | Track false positives and false negatives for native static reports. | Dead code, affected tests, permission audit, and impact analysis all have known static-analysis limitations. | Each report has fixtures for known false positives/negatives and docs expose the current confidence boundaries. |
| D65 | P1 | Add | Define JSON schema and exit-code guarantees for `native-check`. | Native-check is useful for CI and agents but lacks a documented contract. | JSON schema, exit codes, and sample outputs are committed and tested. |
